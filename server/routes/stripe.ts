...
function getStripe(): Stripe {
  const secret = process.env.STRIPE_SECRET_KEY || process.env.VITE_STRIPE_SECRET_KEY || process.env.STRIPE_SECRET;
...
export const createCheckoutSession: RequestHandler = async (req, res) => {
  try {
    ...
    const { amount, currency = "EUR", packId, description, success_url, cancel_url, uid, email } = req.body as any;
